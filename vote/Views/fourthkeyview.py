...
def index(request):
	context = {'pod':fourthdel_groups.objects.all()}
...
def fourthkey_generator(request):
    ...
    if request.method=="POST":
        key=fourthdel_groups()
        member=fourthdel_groups_members()
...
def fourthshow(request,id): 
    elect_obj=fourthdel_groups_members.objects.filter(member_id=request.user.id).values_list("elect_count",flat=True)[0]
    print("1010",elect_obj)
    count_obj = fourthdel_groups_members.objects.filter(member_id=request.user.id).values_list("vote_count",flat=True)[0]
    ...
    if fourthdel_groups.objects.filter(group_owner_id=request.user).exists():
        ...
        owner_id=0       
    approval_obj = fourthdel_groups_members.objects.filter(group_id=key1)
    podlength=len(fourthdel_groups_members.objects.filter(group_id=key1,member_status = 1))
    print(">>>>>>>>>>>>>>>>>>>>>",podlength)
    array=[]
    z=approval_obj
    print("z",len(z))
    for i in z:
        ...
        elif fourthdel_groups_members.objects.filter(member_status=0,member_id=request.user.id,group_id = key1.id):
            break
    ...
    if request.method=="POST" and "submit" in request.POST:
        if fourthdel_groups_members.objects.filter(member_status=0,group_id=key1):
             fourthdel_groups_members.objects.update(vote_given=0)
        member=fourthdel_groups_members() 
        q = request.POST.get('submit')
        voteCount=F('vote_count')+1   
        member.vote_count=fourthdel_groups_members.objects.filter(id=q).update(vote_count=voteCount)  
        member.vote_given=fourthdel_groups_members.objects.filter(member_id=request.user.id).update(vote_given=1)  
        
        show=fourthdel_groups_members.objects.filter(id=q)
        podlen=len(fourthdel_groups_members.objects.filter(group_id=key1,member_status = 1))
        podLen=podlen/2
        length=podLen
        for i in show:
        ...
        if i.vote_count > length:
            print(i.member.id)
            fourthdel_groups_members.objects.filter(member_status =1,group_id=key1)
            # print("true")
            fourthdel_groups_members.objects.update(vote_given=0)
            member.pod_owner_id_id=fourthdel_groups_members.objects.filter(id=q).update(member_status=1)   
    ...
    if request.method=="POST" and "devote" in request.POST:
        ...
        voteCount=F('vote_count')-1   
        member.vote_count=fourthdel_groups_members.objects.filter(id=q).update(vote_count=voteCount)  
        member.devote_given=fourthdel_groups_members.objects.filter(member_id=request.user.id).update(devote_given=1)  
        show=fourthdel_groups_members.objects.filter(id=q)
        podlen=len(fourthdel_groups_members.objects.filter(group_id=key1,member_status = 1))
        podLen=podlen/2
        length=podLen
    ...
    if request.method=="POST" and "Delete" in request.POST:
        ...
        member.count=fourthdel_groups_members.objects.filter(id=q).delete()
        fourthdel_groups_members.objects.filter(member_status =1).update(devote_given=0)
        fourthdel_groups_members.objects.filter(member_status =1).update(vote_given=0)
        return redirect(request.path_info) 

    if request.method == "POST" and "hello" in request.POST:
        z =''.join(random.choices(string.digits, k=6))
        fourthdel_groups.objects.filter(group_owner_id=request.user.id).update(group_key=z)
        return redirect(request.path_info) 
    ...
    if request.method=="POST" and "elect" in request.POST:
        member=fourthdel_groups_members()
        mem=fourthdel_groups()
        q = request.POST.get('elect')
        voteCount=F('elect_count')+1
        member.elect_count=fourthdel_groups_members.objects.filter(id=q).update(elect_count=voteCount)  
        member.elect_vote_given=fourthdel_groups_members.objects.filter(member_id=request.user.id).update(elect_vote_given=1)        
        podlen=len(fourthdel_groups_members.objects.filter(group_id=key1,member_status = 1))
        podLen=podlen/2
        length=podLen
