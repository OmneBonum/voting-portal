...
def tshow(request,id):
    ...
    if request.method=="POST" and "elect" in request.POST:
        ...
        return redirect(request.path_info)   
    return render(request,"key/thirddelkey.html",{'user':users,'key1':key1,'stat':status,'is_key_generate':0,'podlen':podlength,"owner_id":owner_id,'all':all,'votegive':hell,"evote":evote,'r':0,"devote":devotee,'count_obj':count_obj,'elect_obj':elect_obj}) 
