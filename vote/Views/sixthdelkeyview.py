...
def index(request):
	context = {'pod':sixthdel_groups.objects.all()}
...
def sixthkey_generator(request):
    ...
    if request.method=="POST":
        key=sixthdel_groups()
        member=sixthdel_groups_members()
...
def sixthshow(request,id):
    elect_obj=sixthdel_groups_members.objects.filter(member_id=request.user.id).values_list("elect_count",flat=True)[0]
    print("1010",elect_obj)
    count_obj = sixthdel_groups_members.objects.filter(member_id=request.user.id).values_list("vote_count",flat=True)[0]
    ...
    if sixthdel_groups.objects.filter(group_owner_id=request.user).exists():
        ...
        owner_id=0       
    approval_obj = sixthdel_groups_members.objects.filter(group_id=key1)
    podlength=len(sixthdel_groups_members.objects.filter(group_id=key1,member_status = 1))
    print(">>>>>>>>>>>>>>>>>>>>>",podlength)
    array=[]
    z=approval_obj
    print("z",len(z))
    for i in z:
        ...
        elif sixthdel_groups_members.objects.filter(member_status=0,member_id=request.user.id,group_id = key1.id):
            break
    ...
    if request.method=="POST" and "submit" in request.POST:
        if sixthdel_groups_members.objects.filter(member_status=0,group_id=key1):
             sixthdel_groups_members.objects.update(vote_given=0)
        member=sixthdel_groups_members() 
        ...
        member.vote_given=sixthdel_groups_members.objects.filter(member_id=request.user.id).update(vote_given=1)  
        ...
        if i.vote_count > length:
            ...
            # print("true")
            sixthdel_groups_members.objects.update(vote_given=0)
            member.pod_owner_id_id=sixthdel_groups_members.objects.filter(id=q).update(member_status=1)   
    ...
    if request.method=="POST" and "Delete" in request.POST:
        ...
        member.count=sixthdel_groups_members.objects.filter(id=q).delete()
        sixthdel_groups_members.objects.filter(member_status =1).update(devote_given=0)
        sixthdel_groups_members.objects.filter(member_status =1).update(vote_given=0)
        return redirect(request.path_info) 

    if request.method == "POST" and "hello" in request.POST:
        z =''.join(random.choices(string.digits, k=6))
        sixthdel_groups.objects.filter(group_owner_id=request.user.id).update(group_key=z)
        return redirect(request.path_info) 
    ...
    if request.method=="POST" and "elect" in request.POST:
        ...
        member.elect_count=sixthdel_groups_members.objects.filter(id=q).update(elect_count=voteCount)  
        member.elect_vote_given=sixthdel_groups_members.objects.filter(member_id=request.user.id).update(elect_vote_given=1)        
        podlen=len(sixthdel_groups_members.objects.filter(group_id=key1,member_status = 1))
        podLen=podlen/2
        length=podLen
