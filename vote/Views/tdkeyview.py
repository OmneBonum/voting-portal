...
def tkey_generator(request):
    ...
    return render(request,"key/tdelkey.html",{'user':user,'is_key_generate':1})
...
def tshow(request,id):      
    ...
    if request.method=="POST" and "elect" in request.POST:
        ...
        return redirect(request.path_info)   
    return render(request,"key/sdelkey.html",{'user':users,'key1':key1,'stat':status,'is_key_generate':0,'podlen':podlength,"owner_id":owner_id,'all':all,'votegive':hell,"evote":evote}) 
