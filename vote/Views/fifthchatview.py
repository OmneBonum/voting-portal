...
def firoom(request, firoom):
    username = request.GET.get('username')
    # room_details = Room.objects.get(name=room)
    return render(request, 'chat/firoom.html', {
        'username': username,
        # 'room': room,
        # 'room_details': room_details
    })

def ficheckview(request):
...
def figetMessages(request):
    ...
    messages = fifthMessage.objects.filter(room=z)
